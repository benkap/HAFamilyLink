...
### Fixed
- **Standalone: documented the required `/api/cookies` key flow.** The cookie endpoint stays protected because it returns the parent Google session. Docker standalone users can either read the generated key from `./data/api_key` and append `?api_key=<key>` to the integration URL, or set `API_KEY` explicitly.
